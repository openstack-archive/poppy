...
class CheckCertStatusTask(task.Task):
    ...
    def execute(self, cert_obj_json):
        cert_obj = ssl_certificate.load_from_json(cert_obj_json)
        ...
        resp = self.akamai_driver.sps_api_client.get(
...
class UpdateCertStatusTask(task.Task):
    ...
    def execute(self, project_id, cert_obj_json, status_change_to):
        ...
        if status_change_to == "deployed":
            ...
            service_obj.provider_details['Akamai'].domains_certificate_status.\
                set_domain_certificate_status(cert_obj.domain_name, 'deployed')
