...
class ServiceController(base.ServiceBase):
    ...
    def __init__(self, driver):
        ...
        self.driver = driver
        self.policy_api_base_url = self.driver.policy_api_base_url
        self.ccu_api_base_url = self.driver.ccu_api_base_url
        ...
        if len(service_obj.domains) > 0:
            ...
            try:
                for classified_domain in classified_domains:
                    ...
                    LOG.info('Start create new policy %s ' %
                             classified_domain[0])
                    resp = self.policy_api_client.put(
                        self.policy_api_base_url.format(
                            policy_name=classified_domain[0]),
                    ...
                    # TODO(tonytan4ever): leave empty links for now
                    # may need to work with dns integration
                    LOG.info('Creating new policy %s on domain %s complete' %
                             (classified_domain[0], dp))
                links.append({'href': self.driver.akamai_access_url_link,
                              'rel': 'access_url'
                              })
            ...
            for policy in policies:
                ...
                try:
                    LOG.info('Start updating policy %s ' % policy)
