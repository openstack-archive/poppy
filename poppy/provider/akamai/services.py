            ...
            for san_cert_name in self.san_cert_cnames:
                ...
                if resp.status_code != 202:
                    ...
                    self.san_info_storage.save_cert_last_spsid(san_cert_name,
                                                               this_sps_id)
                    self.responder.ssl_certificate_provisioned(san_cert_name, {
                        'status': 'san cert pending',
                        'san cert': san_cert_name,
                        'extra_info': 'Waiting for customer domain validation'
                                      ' for %s' % (cert_obj.domain_name)
                    })
            else:
                self.mod_san_queue.enqueue_mod_san_request(
                    json.dumps(cert_obj.to_dict()))
                self.responder.ssl_certificate_provisioned(None, {
                    'status': 'no availalbe san cert',
                    'san cert': san_cert_name,
                    'extra_info': 'No available san cert for %s right now.'
                                  ' More provisioning might be needed' %
                                  (cert_obj.domain_name)
                })
        else:
            self.responder.ssl_certificate_provisioned(None, {
                'status': 'Cert provisioned failed',
                'extra_info': 'Cert type : %s hasn\'t been implemented'
            })
