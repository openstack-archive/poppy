...
def create_log_delivery_container(project_id, auth_token):
    ...
    public_container_url = '{0}/{1}'.format(swifturl_public, container_name)
    internal_container_url = '{0}/{1}'.format(swifturl_internal,
                                              container_name)
    headers = {'Content-Type': 'application/json',
               'X-Auth-Token': auth_token}
    LOG.info('Starting to create container {0}'.format(public_container_url))
    response = requests.put(public_container_url, None, headers=headers)
    if response.ok:
        ...
        LOG.info('Error creating container {0}'.format(public_container_url))
        return []
