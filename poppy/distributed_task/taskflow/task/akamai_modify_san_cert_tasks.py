...
class PrepareMODSANCertTask(task.Task):
    ...
    def __init__(self):
        ...
        self.added_services = [service for service in self.all_services
                               if service[0] == "add"]
        self.removed_services = [service for service in
                                 self.all_services
                                 if service[0] == "remove"]
    ...
    def execute(self):
        ...
        added_domains = [domain[1] for domain in all_domains
                         if domain[0] == 'add']
        san_cert_mod_input = {}
    ...
    def _gather_service_hosts(self, all_services):
        result = []
        for service_tuple in all_services:
            action, (project_id, service_id) = service_tuple
            services_controller = (
            ...
            for i_id in json.loads(pd.provider_service_id):
                if i_id.get('certificate', None) == 'san':
                    ...
                    result.append((action, i_id.get('policy_name')))
        return result
    ...
    def execute(self, san_cert_mod_input):
        ...
        if resp.status_code != 202:
            ...
            for domain_action_info in self.all_domains:
                action, domain = domain_action_info
