...
class PropertyUpdateTask(task.Task):
    ...
    def execute(self):
        ...
        for papi_job in self.papi_job_list:
            ...
            services_list.extend(papi_job_dict.get('services_list_info', []))
            ...
            if papi_job_dict['j_type'] == 'hosts':
                ...
                self.existing_hosts.extend(
                    json.loads(papi_job_dict['message']))
            if papi_job_dict['j_type'] == 'hosts-remove':
                update_hostnames = True
                pass
                        ...
                        if self.existing_hosts == []:
                            ...
                            resp = papi_api_client.get(
                                papi_base_url.format(
                                    middle_part=(
                                        'poperties/%s/versions/%s/hostnames' %
                                        (self.akamai_conf['property_id'],
                                         str(update_version)))
                                )
                            )
    ...
    def _update_san_cert_file(self, edgehostname, edgehostNameID):
        ...
        for san_cert_info in enumerate(san_cert_list):
