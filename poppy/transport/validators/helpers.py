...
def is_valid_root_domain(domain):
    ...
    # international ltop level domains should have atleast 3 segments
    minimum_segments = '[^.]+\.[^.]+\.[^.]+'

    domain_name = domain.get('domain')

    return (re.search(generic_cc_tld, domain_name) or
...
def is_valid_service_configuration(service, schema):
    ...
    for domain in service['domain']:
        if is_a_root_domain(domain):
            raise exceptions.ValidationFailed(
                u'Domain {0} is a root domain'.format(domain.get('domain')))
