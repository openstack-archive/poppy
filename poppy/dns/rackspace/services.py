...
class ServicesController(base.ServicesBase):
    ...
    def _update_removed_domains():
        # gather the links for the deleted domains
        removed_access_urls = []
        for domain in removed_domains:
            for provider_detail in provider_details:
                access_url = provider_detail['access_urls'][domain]
                removed_access_urls.append(access_url)
        ...
        for provider_name in provider_details:
            ...
            for provider_url in provider_detail.access_urls:
                ...
                if access_url not in removed_access_urls:
                    continue
                try:
    ...
    def update(self, service_old, service_updates, responders):
        ...
        old_domains = sets.Set([domain.domain for domain in service_old.domains])
        new_domains = sets.Set([domain.domain for domain in service_updates.domains])
        if old_domains == new_domains:
            return

        # get the list of added and removed domains
        ...
        # add new domains
        dns_details = self._update_added_domains(responders, added_domains)

        # add previous domains
        provider_details = service_old.provider_details
        for provider_name in provider_details:
            access_urls = provider_details[provider_name].access_urls
            dns_details[provider_name]['access_urls'].update(access_urls)
