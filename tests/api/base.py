...
class TestBase(fixtures.BaseTestFixture):
    ...
    def getProviderUpdates(self, provider, service_name):
        if provider == 'fastly':
    ...
    def assertFlavorUpdated(self, flavor, service_name):
        """Verify response schema aligns with the expected schema."""
        provider_list = self.config.flavor[flavor]
        for provider in provider_list:
            service_details = self.getProviderUpdates(provider, service_name)
