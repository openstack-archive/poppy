...
class TestServicePatch(base.TestBase):
    ...
    def test_patch_service_claim_relinquish_domain(self):
        new_domain = self.generate_random_string(
            prefix='api-test-service') + '.com'

        add_domain = (
            [{
                "op": "add",
                "path": "/domains/-",
                "value": {"domain": new_domain, "protocol": "http"}
            }])
        remove_domain = (
            [{
                "op": "remove",
        ...
        body = resp.json()
        self.assertEqual(body['status'], 'deployed')
        self.assertEqual(body['domains'][-1]['domain'], new_domain)
        ...
        for domain in body['domains']:
            self.assertNotEqual(domain['domain'], new_domain)
    ...
    @ddt.data('FULL', 'MY_CRAZY_STATE')
    def test_patch_service_state(self, new_status):

        patch_state_url = self.service_url + '/state'
        updated_status = {}
        resp = self.client.patch_service(location=patch_state_url,
                                         request_body=updated_status)
        self.assertEqual(resp.status_code, 202)
