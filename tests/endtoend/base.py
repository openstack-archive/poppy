...
class TestBase(fixtures.BaseTestFixture):
    ...
    def setup_cname(self, name, cname):
        ...
        # once we know the name is on the authoritative nameserver, there will
        # still be a delay for the name to propagate out to the caching nameservers.
        sleep_time = 30
        print "waiting {0} additional seconds to propagate to caching nameservers".format(sleep_time)
