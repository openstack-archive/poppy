...
class TestCaching(base.TestBase):
    ...
    def setUpClass(cls):
        ...
        cls.jpg_endpoint = cls.caching_config.jpg_endpoint
        cls.txt_endpoint = cls.caching_config.txt_endpoint
        cls.zip_endpoint = cls.caching_config.zip_endpoint
    ...
    def test_cache_rules(self):
        ...
        jpg_endpoint = self.cacherules_config.jpg_endpoint
        jpg_ttl = self.cacherules_config.jpg_ttl

        txt_endpoint = self.cacherules_config.txt_endpoint
        txt_ttl = self.cacherules_config.txt_ttl

        zip_endpoint = self.cacherules_config.zip_endpoint
        zip_ttl = self.cacherules_config.zip_ttl

        # Setup cache list using cache
        caching_list = [
            {"name": "jpg", "ttl": jpg_ttl,
             "rules": [{"name": "jpg_rule", "request_url": jpg_endpoint}]},
            {"name": "txt", "ttl": txt_ttl,
             "rules": [{"name": "txt_rule", "request_url": txt_endpoint}]},
            {"name": "zip", "ttl": zip_ttl,
             "rules": [{"name": "zip_rule", "request_url": zip_endpoint}]}]
        ...
        cdn_jpg_url = cdn_url + jpg_endpoint
