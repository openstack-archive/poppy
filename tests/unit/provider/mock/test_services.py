...
class MockProviderServicesTest(base.TestCase):
    ...
    def setUp(self, mock_driver):
        ...
        self.test_provider_detail = json.dumps({'id': 73242})
        self.sc = services.ServiceController(self.driver)

    @ddt.file_data('data_service.json')
    def test_update(self, service_json):
        response = self.sc.update(self.test_provider_detail, service_json)
        self.assertTrue(response is not None)

    def test_delete(self):
        response = self.sc.delete(self.test_provider_detail)
        self.assertTrue(response is not None)
